...
# =========================
# SORT ROWS + COLUMNS SEPARATELY
# =========================

# rows = evaluation communities
eval_sorted_pairs = sorted(
    raw_pairs,
    key=lambda x: x[2],
    reverse=True
)

# cols = prompt communities
prompt_sorted_pairs = sorted(
    prompt_raw_pairs,
    key=lambda x: x[2],
    reverse=True
)

eval_sorted_comms  = [p[0] for p in eval_sorted_pairs]
eval_sorted_labels = [p[1] for p in eval_sorted_pairs]

prompt_sorted_comms  = [p[0] for p in prompt_sorted_pairs]
prompt_sorted_labels = [p[1] for p in prompt_sorted_pairs]

eval_idx   = [idx_map[c] for c in eval_sorted_comms]
prompt_idx = [idx_map[c] for c in prompt_sorted_comms]

matrix_dual_sorted = matrix[np.ix_(eval_idx, prompt_idx)]
...
plt.imshow(matrix_dual_sorted, cmap="Blues", vmin=0.5, vmax=1)

plt.xticks(range(n), prompt_sorted_labels, rotation=90)
plt.yticks(range(n), eval_sorted_labels)

plt.xlabel("Prompt Community (sorted by prompt mean accuracy)")
plt.ylabel("Evaluation Community (sorted by evaluation mean accuracy)")
